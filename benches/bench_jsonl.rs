/// Baseline benchmark: JSONL export throughput.
///
/// Run with: `cargo bench --bench bench_jsonl --features jsonl`
...
fn bench_jsonl_export(c: &mut Criterion) {
    let bench_dir = PathBuf::from("target/bench_jsonl");
...
criterion_group!(benches, bench_jsonl_export);
