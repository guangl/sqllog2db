...
impl DuckDBExporter {
    ...
    fn flush_buffer(&mut self) -> Result<()> {
        ...
        debug!("刷新 {} 条记录到 DuckDB 数据库", count);
...
impl Exporter for DuckDBExporter {
    fn initialize(&mut self) -> Result<()> {
        info!(
            "初始化 DuckDB 数据库导出器: path={}, table={}, overwrite={}",
        ...
        let conn = Connection::open(&self.path).map_err(|e| {
            Error::Export(ExportError::DatabaseExportFailed {
                table_name: self.table_name.clone(),
                reason: format!("连接 DuckDB 数据库失败: {}", e),
            })
        })?;
    ...
    fn finalize(&mut self) -> Result<()> {
        ...
        if let Some(conn) = self.connection.take() {
            ...
            info!("成功关闭 DuckDB 数据库连接");
        }

        info!(
            "DuckDB 导出完成: 成功={}, 失败={}, 跳过={}",
...
mod tests {
    ...
    #[test]
    fn test_duckdb_exporter_new() {
        let exporter =
            DuckDBExporter::new("test.duckdb".to_string(), "test_logs".to_string(), true);

        assert_eq!(exporter.path, "test.duckdb");
    ...
    #[test]
    fn test_duckdb_exporter_with_batch_size() {
        let exporter = DuckDBExporter::with_batch_size(
    ...
    fn test_create_table_sql() {
        let exporter = DuckDBExporter::new("test.duckdb".to_string(), "my_table".to_string(), true);
    ...
    #[test]
    fn test_insert_sql() {
        let exporter = DuckDBExporter::new("test.duckdb".to_string(), "logs".to_string(), false);
    ...
    #[test]
    fn test_duckdb_initialize_and_create_table() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");

        let mut exporter = DuckDBExporter::new(
        ...
        // 验证表已创建
        {
            ...
            let mut stmt = conn
                .prepare("SELECT table_name FROM information_schema.tables WHERE table_name = 'test_logs'")
                .unwrap();
    ...
    #[test]
    fn test_duckdb_overwrite_mode() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
    ...
    #[test]
    fn test_duckdb_export_single_record() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
    ...
    #[test]
    fn test_duckdb_export_batch() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
    ...
    #[test]
    fn test_duckdb_batch_flush() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");

        // 设置批量大小为 5
        let mut exporter = DuckDBExporter::with_batch_size(
            db_path.to_str().unwrap().to_string(),
            "logs".to_string(),
            true,
            5,
    ...
    #[test]
    fn test_duckdb_with_null_indicators() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
    ...
    fn test_duckdb_name() {
        ...
        assert_eq!(exporter.name(), "duckdb");
    }

    #[test]
    fn test_duckdb_stats() -> Result<()> {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
