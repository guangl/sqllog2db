...
## Current Milestone: v1.3 SQL 模板分析 & 可视化

**Goal:** 让用户能从 SQL 日志中提取模板级统计信息，并通过 SVG 图表直观呈现执行频率与耗时分布。

**Target features:**
- SQL 模板归一化引擎（统一大小写 + 空白规范化 + 注释去除 + IN 列表统一）
- 模板统计聚合器（pipeline 新处理器，流式累积频率 + 耗时，输出 avg/min/max/p50/p95/p99 + 直方图）
- 双路统计输出（独立 JSON/CSV 报告 + SQLite sql_templates 表 / CSV 伴随文件）
- SVG 图表生成（Top N 条形图、耗时直方图、时间趋势折线图、用户/Schema 饼图）

---

## Previous: v1.2 已发布 ✅ (2026-05-15)
...
- [ ] **TMPL-01**: 用户可通过 config 启用 SQL 模板归一化（统一大小写 + 空白 + 注释 + IN 列表）
- [ ] **TMPL-02**: 用户可在 pipeline 中启用模板统计聚合器，流式处理所有记录后得到每个模板的频率与耗时统计
- [ ] **TMPL-03**: 模板统计结果输出为独立 JSON 或 CSV 报告文件（config 指定路径）
- [ ] **TMPL-04**: SQLite 导出时生成 sql_templates 统计表；CSV 导出时生成 *_templates.csv 伴随文件
- [ ] **CHART-01**: 用户可在 config 中指定 SVG 输出目录，运行后自动生成图表文件
- [ ] **CHART-02**: 生成 Top N 模板频率条形图（SVG）
- [ ] **CHART-03**: 生成耗时分布直方图（SVG，每个模板或全局）
- [ ] **CHART-04**: 生成 SQL 执行频率时间趋势折线图（SVG）
- [ ] **CHART-05**: 生成用户 / Schema 占比饼图（SVG）
...
- Rust LOC: ~11,139 | 测试套件: 729 tests | 基准: ~5.2M records/sec (CSV synthetic)
- Nyquist 审计链：Phase 3/4/5/6 VALIDATION.md 全部 compliant
...
---
*Last updated: 2026-05-15 — milestone v1.3 started*
